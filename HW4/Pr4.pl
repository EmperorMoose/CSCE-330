...
%CSCE330
%Oct152017
% many of these dont need conditions,i.e. the :- or anything after it.

exactly3(X):- X = [A,B,C].

at_least_3(X):- X = [A,B,C|T].

%requiresSeveralPredicates
at_most_3(X):- X = [].
at_most_3(X):- X = [A].
at_most_3(X):- X = [A,B].
at_most_3(X):- X = [A,B,C].

intersect(X,Y):-member(A,X), member(A,Y). all_instersect([],_).

all_intersect([H|T],Y) :- intersect(H,Y),
all_intersect(T,Y).
